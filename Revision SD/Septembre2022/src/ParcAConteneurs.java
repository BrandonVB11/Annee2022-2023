...
public class ParcAConteneurs {
    ...
    public ParcAConteneurs(int nombrePeriodes, int maxVoitures, int maxPeriodes){
        //TODO
        if (nombrePeriodes < 1) {
            throw new IllegalArgumentException("Il n'y a pas au moins une periode");
        }
        if (maxVoitures < 1){
        ...
        for (int i = 0; i < nombrePeriodes; i++) {
            this.tablePeriodes[i] = new HashSet<>();
        }
    }
    ...
    public boolean enregistrerVoiture(String voiture, int numeroPeriode) {
        ...
        if (numeroPeriode < 1 && tablePeriodes.length > numeroPeriode){
            throw new IllegalArgumentException();
        }
        if (mapVoitures.containsKey(voiture)) {
            if (mapVoitures.get(voiture) < maxPeriodes) {
                if (tablePeriodes[numeroPeriode - 1].size() < maxVoitures) {
