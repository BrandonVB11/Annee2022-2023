...
public class ABRDEntiers implements Iterable<Integer> {
	...
	private int nombreNegatifsVR(NoeudEntier noeud){
		...
		count += nombreNegatifsVR(noeud.gauche);
		count += nombreNegatifsVR(noeud.droit);
		return count;
	}
	...
	private boolean tousPositifsVR(NoeudEntier racine){
		if (racine == null){
			return true;
		}
		if (racine.entier < 0){
		...
		return tousPositifsVR(racine.gauche) && tousPositifsVR(racine.droit);
	}
