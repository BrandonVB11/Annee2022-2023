...
public class ListeLRU<E> {
	...
	public boolean insererEnTete (E element){
		//TODO
		Noeud inserer = new Noeud(element);
		if (mapElementNoeud.containsKey(element)){
			return false;
		}
		if (mapElementNoeud.isEmpty()){
			tete.suivant = inserer;
			inserer.precedent = tete;
			queue.precedent = inserer;
			inserer.suivant = queue;
			mapElementNoeud.put(element,inserer);
			return true;
		}
		inserer.suivant = tete.suivant;
		tete.suivant.precedent = inserer;
		tete.suivant = inserer;
		inserer.precedent = tete;
		mapElementNoeud.put(element,inserer);
		return true;
	...
	public boolean supprimer (E element) {
		...
		mapElementNoeud.remove(element,aSupprimer);
		return true;
	}
	...
	public E supprimerDernier() {
		...
		Noeud dernier = queue.precedent;
		dernier.precedent.suivant = queue;
		queue.precedent = dernier.precedent;
		mapElementNoeud.remove(dernier.element,dernier);
