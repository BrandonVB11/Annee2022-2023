...
public class CabinesDEssayage {
	...
	public CabinesDEssayage(int nombreCabines){
		if(nombreCabines<=0)
			throw new IllegalArgumentException("le nombre de cabines est negatif ou nul");
	...
	public void libererCabine(int numeroCabine) throws IllegalArgumentException, IllegalStateException{
		if (numeroCabine <=0||numeroCabine>tableOccupations.length)throw new IllegalArgumentException("aucune cabine porte ce numero");
		if (!tableOccupations[numeroCabine-1])throw new IllegalStateException("la cabine a liberer n'est pas occupee");
