...
public class FileDePrioriteDEntiersImpl implements FileDePrioriteDEntiers {
	...
	private void pushUp(int i){
		//TODO
		// methode recursive
		pushUp(i,indiceParent(i),arbre[taille]);
	}

	private void pushUp(int i,int indiceParent,int entier){
		if(entier <= arbre[indiceParent]){
			return;
		}
		arbre[i]=arbre[indiceParent];
		arbre[indiceParent]=entier;
		i=indiceParent;
		indiceParent=indiceParent(i);
		pushUp(i,indiceParent,entier);
	}
