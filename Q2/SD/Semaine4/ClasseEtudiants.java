...
public class ClasseEtudiants{
	...
	public SerieEtudiants getSerieEtudiants(int numeroSerie)throws IllegalArgumentException{
		// TODO
		if (numeroSerie < 1 || numeroSerie > tableSeries.length)
			throw new IllegalArgumentException();
	...
	public boolean changerSerie(String nom, int nouveauNumeroSerie){
		if (nom == null || nom.equals("") ||nouveauNumeroSerie > tableSeries.length || nouveauNumeroSerie < 0)
			throw new IllegalArgumentException();
		int serieEtudiant = numeroSerie(nom);
		if (serieEtudiant == nouveauNumeroSerie)
			return false;
		tableSeries[serieEtudiant-1].supprimerEtudiant(nom);
