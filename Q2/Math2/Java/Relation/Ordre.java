...
public class Ordre extends RelationAbstraite {
	...
	public void enleverDuSousJacent(Elt x) {
		...
		if (couples.depart().contient(x) && couples.arrivee().contient(x)) {
			Relation tmp = new Relation(couples);
			for (Couple c : tmp) {
	...
	public void ajouter(Couple c) {
		if(c == null) throw new IllegalArgumentException();
		if (!this.depart().contient(c.getX()) ||!couples.arrivee().contient(c.getY())) throw new IllegalArgumentException();
		Relation r = couples.clone();
		r.ajouter(c);
		r.cloTrans();
		if (!r.antisymetrique()) throw new IllegalArgumentException();
		couples = r;
		//TODO
	}
	...
	public boolean comparables(Elt x, Elt y) {
		...
		if (x == null || y == null)
			throw new IllegalArgumentException();
		if (!this.couples.depart().contient(x) || !this.couples.depart().contient(y))
			throw new IllegalArgumentException();
	...
	public EnsembleAbstrait minimaux(EnsembleAbstrait b) {
		...
		for (Elt e : b) {
			for (Couple c : couples) {
				if (c.getY().equals(e) && b.contient(c.getX()) && !c.getX().equals(c.getY())) {
					ajout = false;
					break;
				}
				ajout = true;
	...
	public EnsembleAbstrait maximaux(EnsembleAbstrait b) {
		...
		for (Elt e : b) {
			for (Couple c : couples) {
				if (c.getX().equals(e) && b.contient(c.getY()) && !c.getY().equals(c.getX())) {
					ajout = false;
					break;
				}
				ajout = true;
	...
	public Elt minimum(EnsembleAbstrait b) {
		...
		if (b == null)
			throw new IllegalArgumentException();
		if (minimaux(b).cardinal() > 1 || minimaux(b).estVide())
			return null;
		return minimaux(b).unElement();
	}
	...
	public Elt maximum(EnsembleAbstrait b) {
		...
		if (b == null)
			throw new IllegalArgumentException();
		if (maximaux(b).cardinal() > 1 || maximaux(b).estVide())
			return null;
		return maximaux(b).unElement();
	}
	...
	public EnsembleAbstrait minor(EnsembleAbstrait b) {
		...
		if (!b.inclusDans(couples.depart()))
			throw new IllegalArgumentException();
		Ensemble aRenvoyer = new Ensemble();
		for (Elt elt : couples.depart()) {
			if (b.inclusDans(plusGrandQue(elt)))
	...
	public EnsembleAbstrait major(EnsembleAbstrait b) {
		...
		Ensemble aRenvoyer = new Ensemble();
		for (Elt elt : couples.depart()) {
			if (b.inclusDans(plusPetitQue(elt)))
				aRenvoyer.ajouter(elt);
		}
		if (maximum(b) != null)
	...
	public boolean treillis(){
		//TODO
		for (Elt elt : couples.depart()) {
			for (Elt elt2 : couples.arrivee()) {
				if (!elt.equals(elt2)) {
					Ensemble tmp = new Ensemble();
					tmp.ajouter(elt);
					tmp.ajouter(elt2);
					if (supremum(tmp) == null || infimum(tmp) == null)
						return false;
				}
			}
		}
